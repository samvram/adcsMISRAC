...
void setupinertia()
{
    ...
    I[0][0] = 0.005;    I[0][1] = 0;    I[0][2] = 0;
    I[1][0] = 0;    I[1][1] = 0.018;    I[1][2] = 0;
    I[2][0] = 0;    I[2][1] = 0;    I[2][2] = 0.018;

    /*    I[0][0] = 3.4278;    I[0][1] = 0;    I[0][2] = 0;
    I[1][0] = 0;    I[1][1] = 2.9038;    I[1][2] = 0;
    I[2][0] = 0;    I[2][1] = 0;    I[2][2] = 1.2750;    */
...
typedef struct{
                type A; /* Coil Area */
                type M; /* Coil Mass */
		type N; /* Number of Turns */
                type R,L; /* Resistance and Inductance */
                type rho; /* Resistivity */
		type l,a; /* Wire length and area */
                type *Pos; /* Position vector to coil in brf */
                type *Axis; /* Axis of Coil in brf */
		type Imax; /* Maximum Current that can flow through coil */
		}torquer; 

#define ta 0.1   /*Period of time allotted for PWM pulse actuation*/

/* PWM Characteristics */
#define Vo 5 /* 5 Volt PWM Signals */
#define tp 0.01 /* PWM Sampling cycle */
...
void Init_Torquers()
{
/* Initializing all torquer positions at the origin */
  T[0].Pos = (type*)calloc( 3,sizeof(type) );
  T[0].Axis = (type*)malloc( 3*sizeof(type) );

  T[1].Pos = (type*)calloc( 3,sizeof(type) );
  T[1].Axis = (type*)malloc( 3*sizeof(type) );

  T[2].Pos = (type*)calloc( 3,sizeof(type) );
  T[2].Axis = (type*)malloc( 3*sizeof(type) );

  T[0].Axis[0] = 1;	T[0].Axis[1] = 0;	T[0].Axis[2] = 0;	/* Torquer 1: X(brf) */
  T[1].Axis[0] = 0;	T[1].Axis[1] = 1;	T[1].Axis[2] = 0;	/* Torquer 2: Y(brf) */
  T[2].Axis[0] = 0;	T[2].Axis[1] = 0;	T[2].Axis[2] = 1;	/* Torquer 3: Z(brf) */  
  ...
  for( i=0;i<3;i++ )
  {
    ...
    T[i].N = 782;
    T[i].A = M_PI*pow(4e-2,2);

    T[i].a = 0.13/pow(1e-3,2);
    T[i].l = 2*M_PI*4e-2*T[i].N;
...
void take_etavalues()
{
    ...
    eta[0][0] = 1.e-5;    eta[0][1] = 0;    eta[0][2] = 0;
    eta[1][0] = 0;    eta[1][1] = 1.e-5;    eta[1][2] = 0;
    eta[2][0] = 0;    eta[2][1] = 0;    eta[2][2] = 1.e-5;
}
...
void take_LAMBDAqvalues()
{
    ...
    LAMBDA_q[0][0] = 2.e-6;    LAMBDA_q[0][1] = 0;    LAMBDA_q[0][2] = 0;
    LAMBDA_q[1][0] = 0;    LAMBDA_q[1][1] = 2.e-6;    LAMBDA_q[1][2] = 0;
    LAMBDA_q[2][0] = 0;    LAMBDA_q[2][1] = 0;    LAMBDA_q[2][2] = 2.e-6;

    /*    LAMBDA_q[0][0] = .002;    LAMBDA_q[0][1] = 0;    LAMBDA_q[0][2] = 0;
    LAMBDA_q[1][0] = 0;    LAMBDA_q[1][1] = .002;    LAMBDA_q[1][2] = 0;
    LAMBDA_q[2][0] = 0;    LAMBDA_q[2][1] = 0;    LAMBDA_q[2][2] = .002;   */ 
}
/*---------------*/
type **LAMBDA_s;        /*Positive definite matrix of S in control input; fixed once*/
                        /*Design parameters are fixed*/
type *S;                /*Sliding vector*/
type sang;              /*Angle between required torque and magnetic field*/
