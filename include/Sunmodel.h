#ifndef SUNMODEL_H_INCLUDED
#define SUNMODEL_H_INCLUDED
...
{
    int i;
    double xy_term_sum, sum_psi, sum_epsilon;
    sum_psi = 0;
    sum_epsilon = 0;
...
void sunvector_ECI( type decyear )
{
    spa_data spa;  /*declare the SPA structure*/
    int result;

    type year = (int)decyear;
    type month = (decyear - year)*12;
    type day = (month - (int)month)*30;
    type hour = (day - (int)day)*24;
    type minute = (hour - (int)hour)*60;
    type second = (minute - (int)minute)*60;

    /*Inputs*/
    spa.year          = (int)decyear;
    spa.month         = (int)month+1;
    spa.day           = (int)day+1;
    spa.hour          = (int)hour;
    ...
    if( result==0 )
    {
        sun_ECI = (type*)realloc( sun_ECI,3*sizeof(type) );
        sun_ECI[0] = spa.r*AU;
        sun_ECI[1] = (90-spa.delta)*M_PI/180;
        sun_ECI[2] = spa.alpha*M_PI/180;

        sun_ECI = rtptoxyz( sun_ECI,F );
        free(sun_ECEF);
        sun_ECEF = ECItoECEF( sun_ECI,decyear );

        sun_ECI = arrsum( sun_ECI,arrscalmult(-1,r_sat_eci,3,K),3,F,F );
	free(sun_brf);
        sun_brf = qtransform( q_brf_eci,sun_ECI,K,K );
    }
    else
    {
        printf("Sun Positioning Error\n");
        exit(1);
    }
}
